...
function getIconList(hit) {
	...
	if (typeof(hit.categories)!='undefined') { 
		for (i = 0; i<hit.categories.length; i++) {
	    	...
	    	if (typeof(category)=='string') {
				...
				if (category.indexOf('WGXC Calendar Event')!=-1)
					src = '/images/wgxc-icon.png';
				else if (category.indexOf('Transmission Art')!=-1)
					src = '/images/ta-icon.png';
				else if (category.indexOf('Wave Farm Radio Event')!=-1)
					src = '/images/ta-icon.png';
	...
	// there really only two possible for now, wgxc and ta; if there are two
	// we want wgxc first. Since ta comes first alphabetically we need to reverse order
	iconList.reverse();
