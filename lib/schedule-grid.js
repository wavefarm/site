...
module.exports = function () {
  ...
  if (!table.length) return;
				...
				if (rowSpan>0 && colSpan>0 )	{				
					...
					if (showList.length == 0)
						...
						for(var showIndex=0; showIndex<showList.length; showIndex++) {
							row += '<li><a href="/'+site+'/schedule/'+showList[showIndex].id+'">' 
							row += formatWeeksOfTheMonth(showList[showIndex].weeksOfTheMonth)
							row += showList[showIndex].main 
							row += '</a></li>' 
						}					
					
						var TBAWeeksOfTheMonth = getTBAWeeksOfTheMonth(showList)
						if (TBAWeeksOfTheMonth) {
							row += '<li>' + TBAWeeksOfTheMonth + 'TBA</li>' 
						}
					}
	...
	// returns formatted list of missing weeks of the month
	function getTBAWeeksOfTheMonth(showList) {
		var filledWeeks = [0,0,0,0,0]		
		for (var i=0; i<showList.length; i++) {
			var show = showList[i]
			if (show.weeksOfTheMonth.length == 0)
				return ''
			else
				for (var j=0; j<show.weeksOfTheMonth.length; j++) {
					filledWeeks[show.weeksOfTheMonth[j]] = 1
				}			
		}				
		var weeksOfTheMonth = []
		for (var i=0; i<filledWeeks.length; i++) {
			if (filledWeeks[i] == 0)
				weeksOfTheMonth.push(i);
		}		
		return formatWeeksOfTheMonth(weeksOfTheMonth)
	}
