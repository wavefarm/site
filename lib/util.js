...
/* Formats time only for calendar card, and elsewehre. Only
 * returns time info relevant to first day or event. So if
 ...
 * Also suppresses an endTime of 23:59:59.
 */
	...
	if (typeof(hit.start)!='undefined') {
		if (hit.start.indexOf('T')>-1 && hit.start.substring(hit.start.indexOf('T')).length>1) {
		  startTime = moment(hit.start);	
			endTime = typeof(hit.end)!='undefined'?moment(hit.end):false;
		}
	}
