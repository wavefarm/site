...
module.exports = function () {
		...
		submitHandler: function(form) {
			...
			if (typeof(obj.startDate)!='undefined' && obj.startDate.length>0) {
				...
				if (typeof(obj.startTime)!='undefined' && obj.startTime.length>0) {
					...
					// check for alternate time formats
					var test =  moment(moment().format('YYYY-MM-DDT') + obj.startTime,'YYYY-MM-DDTh:mm A');
					if (test && test.isValid())
						obj.startTime = test.format('HH:mm');
					else {
						test =  moment(moment().format('YYYY-MM-DDT')+ obj.startTime,'YYYY-MM-DDTh:mmA');
						if (test && test.isValid())
							obj.startTime = test.format('HH:mm');
					}
			...
			if (typeof(obj.endTime)!='undefined' && obj.endTime.length>0) {
				...
				// check for alternate time formats
				var test =  moment(moment().format('YYYY-MM-DDT') + obj.endTime,'YYYY-MM-DDTh:mm A');
				if (test && test.isValid())
					obj.endTime = test.format('HH:mm');
				else {
					test =  moment(moment().format('YYYY-MM-DDT') + obj.endTime,'YYYY-MM-DDTh:mmA');
					if (test && test.isValid())
						obj.endTime = test.format('HH:mm');
				}
    ...
    if (hit.locations) {  		
  		...
  		location.find('strong').attr('class','locationName'+hit.locations[0].id);  
  		//longDescription.find('div.location-url').attr('class','location-url-'+hit.locations[0].id);
  		longDescription.find('div.url a').attr('id','location-url-'+hit.locations[0].id);
  		$.getJSON( '/api/'+hit.locations[0].id, function( locationResult ) {
  			...
  			if (locationResult.url && !hit.url) {
  				// use the event url (further below) if present, otherwise a location url (here) if present
		    	var linkDiv = longDescription.find('#location-url-'+locationResult.id);
		    	linkDiv.attr('href',locationResult.url);
		    	linkDiv.html(locationResult.url);  		
  			}
     	});    		    		
  	}	
