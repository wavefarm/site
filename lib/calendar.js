...
module.exports = function () {
	...
	// TODO: needs to be tested for ULRs of form /calendar/YYY-MM-DD, i.e. main site cal
	...
	function renderDayOfEvents () {
   		...
   		$.getJSON( '/api/search?q=type:event'+sites+'&sort=start&size=50&date='+date.format('YYYY-MM-DD'), function( result ) {
	        ...
	        $.each( result.hits, function( i, hit ) {
	        	...
	        	} else if (startDate.diff(date,'days')<0) {
	...
	function renderEvent(event,hit) {				
		...
		event.find('.time').html(util.formatTimes(hit));		
	...
	function renderBasicEventInfo(event,hit,isOngoing) {
		...
		event.find('.event-dates strong').html( util.formatDates(hit));    		
    ...
    // build the short-description
  	...
  	event.on('click', function () {
    	...
    	var img = $(this).find('.event-description-long img');
