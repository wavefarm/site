...
module.exports = function () {
    ...
    if (hit.locations) {  		
  		...
  		$.getJSON( '/api/'+hit.locations[0].id, function( locationResult ) {
  			...
  			if (locationResult.url) {
  				/*
  				$('.locationName'+locationResult.id).html(
  						'<a target="_blank" href="'+locationResult.url+'">' + locationResult.main + '</a>');
  				*/
  				$('.location-url-'+locationResult.id).html(
  						'<a target="_blank" href="'+locationResult.url+'">' + locationResult.url + '</a>');  				
  			}
     	});    		    		
  	}	
