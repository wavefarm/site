...
module.exports = function () {
	  ...
	  function updatePartnerStreams(station) {
		  ...
		  $.getJSON( partnerIcecastStatusUrl, function( result ) {	
			  ...
			  for (i=0; i<streams.length; i++) {				  
				  ...
				  if (stream.title) {
					  streamDiv.find('.listen-nowplaying a').html(stream.title)
				  }
				  else {
					  streamDiv.find('.listen-nowplaying-list').remove()
				  }
			  }			  			  
	  ...
	  function showNowPlayingResults(station,hits,now) {
		  ...
		  for (i=0; i<hits.length; i++) {
			  ...
			  // as well as "extra web show" stuff
			  if (endLocal.diff(startLocal,'days')<=1 && 
					  (typeof(hits[i].shows)=='undefined' || hits[i].shows[0].id != extraWebShowId)) {
