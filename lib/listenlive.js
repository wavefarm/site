...
module.exports = function () {
	  ...
	  $(".listen-wgxc-window").click(function() {
		  window.open('/listen/wgxc','newwindow',
				  'width=350, height=500, toolbar=no, menubar=no, location=no, directories=0, status=0,scrollbar=0,resize=0');
		  return false;
	  });    
	  ...
	  // deal with nowPlaying and upNext via wgxc schedule
	  ...
	  $.getJSON( '/api/wgxc/schedule/'+yesterday.format('YYYY-MM-DD'), function( result ) {
		  ...
		  $.getJSON( '/api/wgxc/schedule/'+now.format('YYYY-MM-DD'), function( result ) {
			  ...
			  $.getJSON( '/api/wgxc/schedule/'+tomorrow.format('YYYY-MM-DD'), function( result ) {
				  ...
				  if (nowPlaying!=null) {
					  $('.listen-nowplaying a').html(nowPlaying.main+':'+nowPlaying.shows[0].main);
					  $('.listen-nowplaying a').attr('href','/wgxc/schedule/'+nowPlaying.id);
					  /*
					  var d = nowPlaying.description || '';
					  d = strip(d);
					  if (d.length > 60) d = d.substr(0, 60) + '...';			  			  
					  $('.listen-nowplaying-desc').html(d);
					  */
				  }
				  else {
					  $('.listen-nowplaying').remove();
					  $('.listen-nowplaying-desc').remove();
				  }
				  
				  if (upNext!=null) {
					  $('.listen-upnext a').html(upNext.main+':'+upNext.shows[0].main);
					  $('.listen-upnext a').attr('href','/wgxc/schedule/'+upNext.id);
					  /*
					  d = upNext.description || '';
					  d = strip(d);
					  if (d.length > 60) d = d.substr(0, 60) + '...';			  			  
					  $('.listen-upnext-desc').html(d);
					  */
				  }
				  else {
					  $('.listen-upnext').remove();
					  $('.listen-upnext-desc').remove();
				  }				  
