...
module.exports = function () {
	...
	$.getJSON( '/api/'+id, function( itemJson ) {
		...
		//$('.event-dates strong').html(wgxc.formatDates(itemJson));
        ...
        $.each( associated, function( i, hit ) {
			...
			$.getJSON( '/api/'+hit.id, function( associatedJson ) {
				...
				if (typeof(associatedJson.id)!='undefined') {
					...
					if (type=='ARTISTS')
						itemArchiveEntry.find('.item-archive-entry-main a').attr('href','http://transmissionarts.org/artist/'+associatedJson.id);
					else
